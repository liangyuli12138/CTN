...
# 2022-12-31
# add feature plot gene marker to help
DefaultAssay(dat) <- "RNA"
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_Tcell_FeaturePlot.pdf",useDingbats=F)
DimPlot(dat,label=T,label.size=6,raster=T)
FeaturePlot(dat,features=c("CD8A","CD4","CD3D","CD8B"),label=T,label.size=6,raster=T)
FeaturePlot(dat,features=c("CCR7","SELL","TCF7","LEF1"),label=T,label.size=6,raster=T) # naive/central memory 
FeaturePlot(dat,features=c("GZMA","GZMB","GZMK","IFNG"),label=T,label.size=6,raster=T) # effector
FeaturePlot(dat,features=c("CTLA4","FOXP3","IL2RA","IKZF2"),label=T,label.size=6,raster=T) # Treg
FeaturePlot(dat,features=c("HAVCR2","PDCD1","VCAM1","TIGIT"),label=T,label.size=6,raster=T) # exhausted
dev.off()

pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_Tcell_FeaturePlot_supp.pdf",useDingbats=F)
DimPlot(dat,label=T,label.size=6,raster=T)
FeaturePlot(dat,features=c("CD3D","CD4","CD8A","CD8B"),label=F,label.size=6,raster=T)
FeaturePlot(dat,features=c("CCR7","SELL","TCF7","LEF1"),label=F,label.size=6,raster=T) # naive/central memory 
FeaturePlot(dat,features=c("GZMA","GZMB","GZMK","IFNG"),label=F,label.size=6,raster=T) # effector
FeaturePlot(dat,features=c("CTLA4","FOXP3","IL2RA","PDCD1"),label=F,label.size=6,raster=T) # Treg
# FeaturePlot(dat,features=c("HAVCR2","PDCD1","VCAM1","TIGIT"),label=T,label.size=6,raster=T) # exhausted
dev.off()
...
# 2022-12-31
# set the new classify 
# more OK
dat$celltype.refine <- "unclassify"
dat$celltype.refine[which(dat$seurat_clusters%in%c(1,2,4,8,10,13))] <- "CD8+ Tem"
dat$celltype.refine[which(dat$seurat_clusters%in%c(11))] <- "γδT"
dat$celltype.refine[which(dat$seurat_clusters%in%c(0))] <- "CD4+ CTL"
dat$celltype.refine[which(dat$seurat_clusters%in%c(3,6))] <- "CD4+ Tcm"
dat$celltype.refine[which(dat$seurat_clusters%in%c(7))] <- "CD8+ Tn"
dat$celltype.refine[which(dat$seurat_clusters%in%c(12))] <- "CD4+ Tn"
dat$celltype.refine[which(dat$seurat_clusters%in%c(5))] <- "CD4+ Tm"
dat$celltype.refine[which(dat$seurat_clusters%in%c(9))] <- "Tregs"
dat$celltype.refine[which(dat$seurat_clusters%in%c(14))] <- "Unclassify"
saveRDS(dat,file="/public/workspace/lily/CTN/version_3_20/data/Tcell.RDS")
...
#   CD4+ CTL   CD4+ Tcm    CD4+ Tm    CD4+ Tn   CD8+ Tem    CD8+ Tn      Tregs
      5680       6813       2952        499      17779       2744       1896
# Unclassify        γδT

cols.t <- c("#00b2a9","#0066a1","#a626aa","#6639b7","#00ad45","#70b29c","#3a4958","#bfbfbf","#aea400")
...
# 2022-12-31
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_overview_new.pdf",width=10,height=10,useDingbats=F)
cols.t <- c("#00b2a9","#0066a1","#a626aa","#6639b7","#00ad45","#70b29c","#3a4958","#bfbfbf","#aea400")
DimPlot(dat,group.by="celltype.refine",cols=cols.t,raster=T)
dev.off()
...
# plot river plot 
# cols.t <- c("#4a8594","#faae40","#598c14","#b5c327","#3a4958","#dbe0e3")
cols.t <- c("#00b2a9","#0066a1","#a626aa","#6639b7","#00ad45","#70b29c","#3a4958","#bfbfbf","#aea400") # 2022-12-31

tmp.res$group <- factor(tmp.res$group,levels=c("Elder","CTN","SC"))

pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_celltype_percent_new.pdf",useDingbats=F,height=10)
...
sub.dat <- subset(dat,cells=colnames(dat)[which(dat$celltype.refine%in%c("CD4+ CTL","CD4+ Tcm","CD4+ Tm","CD4+ Tn","CD8+ Tem","CD8+ Tn","Tregs","γδT"))])
...
# update trajectory
library(monocle)
tmp.dat <- readRDS("/public/workspace/lily/CTN/version_3_20/monocle/Tcell_pure_trajectory.RDS")
all(colnames(tmp.dat)==colnames(sub.dat))
tmp.dat$celltype.refine <- sub.dat$celltype.refine
pdf("/public/workspace/lily/CTN/version_3_20/monocle/Tcell_pure_trajectory_new.pdf",useDingbats=F)
plot_cell_trajectory(tmp.dat,color_by="celltype.refine")+scale_colour_manual(values= c("#00b2a9","#0066a1","#a626aa","#6639b7","#00ad45","#70b29c","#3a4958","#aea400"))
...
# 2023-1-2
# use heatmap to plot result, show gene marker 

genes <- c("CD4","CD8A","CD8B",
        "GZMA","GZMB","GZMK","GZMH","NKG7","IFNG",
        "SELL","CCR7","IL7R","TCF7","LEF1",
        "CTLA4","FOXP3","IL2RA",
        "CD28","ICOS","TNFRSF14","TNFRSF9"
        )
...
library(pheatmap)
dat <- readRDS("/public/workspace/lily/CTN/version_3_20/data/Tcell.RDS")
dat@active.ident <- factor(dat$celltype.refine)
pdat <- data.frame(as.matrix(AverageExpression(dat,features=genes,assays="RNA")$RNA))
pdat <- pdat[,-which(colnames(pdat)=="Unclassify")] # delete unclassify

# range cols
pdat <- pdat[,c("CD4..CTL","CD8..Tem","γδT","CD8..Tn","CD4..Tcm","CD4..Tm","CD4..Tn","Tregs")]

p <- pheatmap::pheatmap(pdat,scale="row",cluster_rows=F,cluster_cols=F,
color=colorRampPalette(c('steelblue','white','red'))(50))

pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_heatmap_genes.pdf",useDingbats=F)
p
dev.off()
...
#=====================================================================================================================================================
# 2023-1-2
# plot CD4+ CTL and CD8+Tem

CD4CTL <- apply(table(dat$sample,dat$celltype.refine),1,function(x){x/sum(x)})[1,]
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_boxplot_CD4CTL.pdf",useDingbats=F)
boxplot(CD4CTL[1:4],CD4CTL[12:14],CD4CTL[5:11])
stripchart(list(CD4CTL[1:4],CD4CTL[12:14],CD4CTL[5:11]),vertical = T,method = "jitter",cex = 0.8,
           pch = 20,col = "red",add=T)
dev.off()

CD8TEM <- apply(table(dat$sample,dat$celltype.refine),1,function(x){x/sum(x)})[5,]
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_boxplot_CD8Tem.pdf",useDingbats=F)
boxplot(CD8TEM[1:4],CD8TEM[12:14],CD8TEM[5:11])
stripchart(list(CD8TEM[1:4],CD8TEM[12:14],CD8TEM[5:11]),vertical = T,method = "jitter",cex = 0.8,
           pch = 20,col = "red",add=T)
dev.off()
...
tmp.res <- t(apply(tmp.f,1,function(x){
    c(mean(x[1:5]),mean(x[c(6:8)]),mean(x[c(9:13)]),mean(x[c(14)]))  
}))
colnames(tmp.res) <- c("SC.CTN","Elder","Uniform","CTN")
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_Tcell_metabolism_new.pdf",height=12,width=8)
pheatmap::pheatmap(tmp.res,scale="row",color=colorRampPalette(c('steelblue','white','red'))(50))
dev.off()


# plot by another way by circle 
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_Tcell_metabolism_circle_new.pdf",width=20,height=10,useDingbats=F)
...
# 2022-12-27
# response use Addmodulescore to calculate 
# same result 
sub.dat <- AddModuleScore(sub.dat,features=list(genes),name="Tcell_act")

pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_Tcell_act_AddModulescore.pdf",useDingbats=F)
boxplot(Tcell_act1~group,data=sub.dat@meta.data,mian="MSGDB T cell act signature",outline=F)
dev.off()
...
# 2023-1-5
# calculate CD4+ T cell percentage

library(Seurat)
dat <- readRDS("/public/workspace/lily/CTN/version_3_20/data/Tcell.RDS")
dat$Tsub <- "Others"
dat$Tsub[which(dat$celltype.refine%in%c("CD4+ CTL","CD4+ Tcm","CD4+ Tm ","CD4+ Tn","Tregs"))] <- "CD4T"

CD4T <- apply(table(dat$sample,dat$Tsub),1,function(x){x/sum(x)})[1,]
pdf("/public/workspace/lily/CTN/version_3_20/rs_plot/Fig2_boxplot_CD4T.pdf",useDingbats=F)
boxplot(CD4T[1:4],CD4T[12:14],CD4T[5:11])
stripchart(list(CD4T[1:4],CD4T[12:14],CD4T[5:11]),vertical = T,method = "jitter",cex = 0.8,
           pch = 20,col = "red",add=T)
dev.off()
